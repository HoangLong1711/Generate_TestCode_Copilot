...
6. **Iterate test improvements (max 5 attempts for coverage only)** — If coverage < 100%, refine test cases for uncovered paths, rebuild (using unlimited auto-fix for any new build errors in test code), re-test, and re-measure coverage; **after 5 coverage improvement iterations**, automatically analyze the coverage gaps to determine why they cannot be reached. Document these limitations by appending detailed explanations to the central `reports/report/coverage_limitations.txt` file under a new heading for the module. The documentation MUST clearly explain the specific reasons—whether the code flow cannot meet the requirements, what specific conditions prevent access, unreachable loops, or STL exception paths. (DO NOT modify the generated HTML report). See `Testing_Rules.md` for the exact format requirement.
