...
6. **Iterate test improvements (max 5 attempts for coverage only)** — If coverage < 100%, refine test cases for uncovered paths, rebuild (using unlimited auto-fix for any new build errors in test code), re-test, and re-measure coverage; **after 5 coverage improvement iterations**, document all uncovered lines with detailed explanations specifying exact code snippets and conditions (unreachable code, conditional complexity, STL exception paths, etc.) in a separate `coverage_limitations.txt` file (DO NOT modify the generated HTML report). See `Testing_Rules.md` for the exact format requirement.
